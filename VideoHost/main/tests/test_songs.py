...
@override_settings(DROPBOX_ACCESS_TOKEN=getattr(settings, 'DROPBOX_ACCESS_TOKEN'))
class TestSongsCase(test.TransactionTestCase):

    def setUp(self):

        self.audio_bytes_content = bytes()
        self.preview = django.core.files.images.ImageFile(file='avatar.jpeg', name='test_preview')
        ...
        email='TestEmail', password='TestPassword', phone_number='89765896865')
    ...
    def dropbox_TearDown(dropbox_app: dropbox.Dropbox) -> bool:
        ...
        headers={'Authorization': "Basic %s %s" % (getattr(settings, 'DROPBOX_ACCESS_KEY'),
        getattr(settings, 'DROPBOX_ACCESS_SECRET'))})
