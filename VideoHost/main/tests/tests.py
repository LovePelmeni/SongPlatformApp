...
class BaseRequestEndpointTestCase(TestCase):
    ...
    def test_endpoints(self, testdir, client):
        result_output = testdir.makepyfile(
        """
            from django import test 
            ...
            def test_create_endpoint(created_data: dict, client):
        ...
        """ % (model, model.lower(), model, model, model, model)
        )
        assert result_output.stdout.files[-4:-2] == [
            u'test_endpoints.py::test_create_endpoint PASSED',
            u'test_endpoints.py::test_update_endpoint PASSED',
            u'test_endpoints.py::test_delete_endpoint PASSED',
        ]
